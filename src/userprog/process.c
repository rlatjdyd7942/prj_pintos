...
load (const char *args, void (**eip) (void), void **esp) 
{
  ...
  process_activate ();
	...
	if ((file_name = strtok_r(args, " ", &tok)) == NULL) {
		printf("load : %s: error parsing arguments\n", args);
		goto done;
	}
  ...
  if (!setup_stack (esp))
	...
	*esp -= (uint32_t)(*esp) % sizeof(uint32_t);	// word align
	*esp -= sizeof(uint32_t);	// end of addr of argv
	...
	*esp += sizeof(uint32_t);
	temp_esp += strlen(file_name) + 1;
	//printf("temp_esp : %x\n", temp_esp);

	while ((args = strtok_r(NULL, " \t\n", &tok)) != NULL) {
		strlcpy(temp_esp, args, strlen(args) + 1);
		*(uint32_t*)(*esp) = temp_esp;
		temp_esp += strlen(args) + 1;
		*esp += sizeof(uint32_t);
	}

	*esp -= argc * sizeof(uint32_t); // argv[4] to argv[0]
	*esp -= sizeof(uint32_t);		//argv
	*(uint32_t*)(*esp) = *esp + sizeof(uint32_t);
	*esp -= sizeof(uint32_t);		//argc
	*(uint32_t*)(*esp) = argc;
